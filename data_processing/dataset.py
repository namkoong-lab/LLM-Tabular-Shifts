import numpy as np
from whyshift import folktables_utils
from whyshift.folktables import ACSDataSource, ACSEmployment, ACSIncome, ACSPublicCoverage, ACSMobility, ACSTravelTime
from whyshift.folktables import BasicProblem
import pandas as pd
from sklearn import preprocessing
import pickle
from sklearn.preprocessing import StandardScaler
from whyshift.folktables_utils import add_indicators, add_indicators_year, add_indicators_pubcov, add_indicators_traveltime, add_indicators_mobility
from whyshift.utils import preprocess
...
def get_raw_df_ACSIncome(state, year=2018, root_dir = './datasets/acs'):
    data_name = 'ACSIncome-{}-{}-raw'.format(state, year)
    try:        # try loading df
        raw_df = pd.read_csv(root_dir+'/raw/{}.csv'.format(data_name), sep=',')
    except:     # create raw df 
        task = ACSIncome
        ...
        # preprocess df
        feature_names =  ['age', 'class of worker', 'educational attainment', 'marital status', 
                          'occupation', 'relationship to householder', 'usual hours worked per week last year', 
                          'sex', 'race']
        new_task = BasicProblem(feature_names, task._target, task._target_transform,
                    task._group, task._group_transform,
                    preprocess = preprocess_ACSIncome, postprocess = task._postprocess)
        variables, target, group = new_task.df_to_pandas(source_data)
        raw_df = pd.concat([variables, target, group], axis=1)
        # save df
        raw_df.to_csv(root_dir+'/raw/{}.csv'.format(data_name), sep=',', index = False)
    return raw_df
...
def get_raw_df_ACSPubCov(state, year=2018, root_dir = './datasets/acs'):
    data_name = 'ACSPubCov-{}-{}-raw'.format(state, year)  
    try:        # try loading df
        raw_df = pd.read_csv(root_dir+'/raw/{}.csv'.format(data_name), sep=',')
    except:     # create raw df 
        ...
        # preprocess df        
        feature_names =  ['age', 'educational attainment', 'marital status', 'sex', 'disability', 
                            "employment status of parents", "citizenship", 'mobility', 'military service', 
                            'ancestry', 'nativity',
                            'hearing difficulty', 'vision difficulty', 'cognitive difficulty',
                            "employment status", "person's total annual income",  
                            "gave birth to child within the past 12 months", "race"]
        new_task = BasicProblem(feature_names, task._target, task._target_transform,
                            task._group, task._group_transform,
                            preprocess = preprocess_ACSPubCov, postprocess = task._postprocess)
        variables, target, group = new_task.df_to_pandas(source_data)
        raw_df = pd.concat([variables, target, group], axis=1)
        # save df
        raw_df.to_csv(root_dir+'/raw/{}.csv'.format(data_name), sep=',', index = False)
...
def get_raw_df_ACSMobility(state, year=2018, root_dir = './datasets/acs'):
    data_name = 'ACSMobility-{}-{}-raw'.format(state, year)  
    try:        # try loading df
        raw_df = pd.read_csv(root_dir+'/raw/{}.csv'.format(data_name), sep=',')
    except:     # create raw df 
        task = ACSMobility
        ...
        # preprocess df
        feature_names = ['age', 'educational attainment', 'marital status', 'sex', 'disability', 
                        "employment status of parents", "citizenship", 'military service',
                        'ancestry', 'nativity', 'relationship to householder',
                        'hearing difficulty', 'vision difficulty', 'cognitive difficulty',
                        'race', "grandparent living with grandchildren",
                        'class of worker', "employment status", 'usual hours worked per week last year', 
                        "travel time to work", "person's total annual income"]
        new_task = BasicProblem(feature_names, task._target, task._target_transform,
                    task._group, task._group_transform,
                    preprocess = preprocess_ACSMobility, postprocess = task._postprocess)
        variables, target, group = new_task.df_to_pandas(source_data)
        raw_df = pd.concat([variables, target, group], axis=1)
        # save df
        raw_df.to_csv(root_dir+'/raw/{}.csv'.format(data_name), sep=',', index = False)
...
def get_raw_df(task_name, state, root_dir, year=2018):
    if task_name == 'income':
        return get_raw_df_ACSIncome(state, year, root_dir)
    elif task_name == 'income_aug':
        return get_ACSIncome_aug(state, year, root_dir)
    elif task_name == 'pubcov':
        return get_raw_df_ACSPubCov(state, year, root_dir)
    elif task_name == 'mobility':
        return get_raw_df_ACSMobility(state, year, root_dir)
    elif task_name == 'accident':
        return get_USAccident(state, root_dir)
    elif task_name == 'taxi':
        return get_taxi(state, root_dir)